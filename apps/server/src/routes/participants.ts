...
import { getAccessibleMeeting, getAccessibleParticipant } from '../lib/access'
...
participantsRouter.post('/', async (c) => {
  ...
  db.insert(participants)
    ...
    .run()
  return c.json(getAccessibleParticipant(c.get('user'), id), 201)
})

// 更新人员
participantsRouter.patch('/:id', async (c) => {
  const p = getAccessibleParticipant(c.get('user'), c.req.param('id'))
  ...
  db.update(participants)
    ...
    .run()
  return c.json(getAccessibleParticipant(c.get('user'), p.id))
})
