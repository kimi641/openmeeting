...
import { db } from '../db'
import { meetings, sessions, venues } from '../db/schema'
...
function getVenueOr404(id: string) {
...
venuesRouter.post('/', async (c) => {
  ...
  db.insert(venues)
    ...
    .run()
  return c.json(getVenueOr404(id), 201)
})
...
venuesRouter.post('/reorder', async (c) => {
  ...
  if (input.venueIds.some((id) => !known.has(id))) throw notFound('场地')
...
venuesRouter.patch('/:id', async (c) => {
  const venue = getVenueOr404(c.req.param('id'))
  ...
  db.update(venues)
    ...
    .run()
  return c.json(getVenueOr404(venue.id))
})
